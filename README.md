...
系统支持在线 LLM API，也支持离线模式。在线模式下，LLM 会基于本地知识库、故障案例库、日志规则库和配置规则库生成结构化回答；离线模式下，系统会根据命中的本地证据生成模板化分析结果。
...
或者不配置 `LLM_API_KEY`，系统会自动进入离线模式。离线模式仍会使用本地知识库、故障案例库、日志规则库和配置规则库生成分析结果。
