...
这是一个在线优先的课程大作业 Demo：Streamlit 负责 Web 页面，Python 后端串联本地知识库、故障案例库、日志规则库和 OpenAI-compatible LLM API。没有 API Key 或在线调用失败时，系统会自动使用离线兜底报告。
...
| 方案要求 | 当前实现 |
| --- | --- |
| Streamlit Web 界面 | `app.py`，支持问题类型选择、示例问题、报告展示、证据区、Markdown 下载 |
...
第一版仍按方案规划暂不实现：自动修改 free5GC/Open5GS 配置、自动重启网元、完整 pcap 解析、完整 3GPP 标准导入、生产级 LangGraph 多 Agent 编排。这些适合作为报告中的“后续工作”。
